...
class TemplatePowerParser
{
  ...
  private   $tpl_include;           //Array( [filename/varcontent], [T_BYFILE/T_BYVAR] ) 
  private   $tpl_count;

  protected $parent   = Array();    // $parent[{blockname}] = {parentblockname}
  protected $defBlock = Array();

  private   $rootBlockName;
  private   $ignore_stack;
  ...
  private   $version;
   ...
   protected function prepare()
   {
	 ...
	 * @access private
     */
    private function prepareTemplate( $tpl_file, $type )
    {
        ...
        else
        {
            $this->{$tplvar}["content"] = @file( $tpl_file ) or
                die( $this->errorAlert('TemplatePower Error: Couldn\'t open [ '. $tpl_file .' ]!'));
        }
	 ...
	 * @access private
     */
    private function parseTemplate( $tplvar, $blockname, $initdev )
    {
...
class TemplatePower extends TemplatePowerParser
{
  ...
  private $prepared;
	 ...
	 * @access private
     */
    private function deSerializeTPL( $stpl_file, $type )
    {
        if( $type == T_BYFILE )
        {
            $serializedTPL = @file( $stpl_file ) or
                die( $this->errorAlert('TemplatePower Error: Can\'t open [ '. $stpl_file .' ]!'));
        }
        else
        {
	 ...
	 * @access private
     */
    private function makeContentRoot()
    {
	 ...
	 * @access private
     */
    private function assignPrivate( $varname, $value)
    {
	 ...
	 * @access private
     */
    private function assignGlobalPrivate( $varname, $value )
    {    	
	 ...
	 * @access private
     */
    private function outputContent( $blockname )
    {
